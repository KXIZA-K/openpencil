...
pub(crate) fn validate_node(node: &Value) -> Result<(), ActionError> {
    for key in ["states", "initialState", "demoOnly", "noNetwork"] {
        if node.get(key).is_some() {
            return Err(ActionError::Custom(format!(
                "Unsupported interaction property '{key}'"
            )));
        }
    }
    if let Some(events) = node.get("events").filter(|v| !v.is_null()) {
        let events = events
            .as_object()
            .ok_or_else(|| ActionError::Custom("events must be an object".into()))?;
        ...
        for (hook, actions) in events {
            ...
            if !actions.is_null() {
                registry.borrow().parse_list(actions)?;
            }
        }
    }
    if let Some(bindings) = node.get("bindings").filter(|v| !v.is_null()) {
        let bindings = bindings
            .as_object()
            .ok_or_else(|| ActionError::Custom("bindings must be an object".into()))?;
        for (property, expression) in bindings {
            let source = expression.as_str().ok_or_else(|| {
                ActionError::Custom(format!("Binding '{property}' must be an expression string"))
            })?;
            jian_core::expression::Expression::compile(source).map_err(|_| {
                ActionError::Custom(format!("Invalid expression in binding '{property}'"))
            })?;
        }
    }
...
pub(crate) fn validate_modifications(
    ops: &[crate::chat_canvas_tools::DesignModificationOp],
) -> Result<(), ActionError> {
    for (_, node) in ops {
        validate_node(
            if node.get("op").and_then(Value::as_str) == Some("update") {
                &node["data"]
            } else {
                node
            },
