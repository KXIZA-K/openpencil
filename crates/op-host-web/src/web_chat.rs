...
pub(crate) fn drain_chat_flags<C: RepaintContext + 'static>(inner: &Rc<RefCell<C>>) {
    if let Ok(mut shell) = inner.try_borrow_mut() {
        ...
        if std::mem::take(&mut chat.pending_history) {
            if let (Some(thread), Some(before)) = (&chat.thread_id, chat.history_before) {
                crate::platform_chat_bridge::request_history(thread, before);
            }
        }
    }
    let (new_chat, stop) = {
    ...
    if new_chat {
        if let Some(thread_id) = crate::platform_chat_bridge::create_thread() {
            if let Ok(mut b) = inner.try_borrow_mut() {
                ...
                chat.pending_thread_creation = true;
...
fn start_pump<C: RepaintContext + 'static>(
    ...
    let tick: Rc<dyn Fn() -> bool> = Rc::new(move || {
        ...
        loop {
            ...
            if event_terminal {
                ...
                // erase the completion payload or mutate a completed message.
                break;
            }
        }
        if changed {
    ...
    crate::raf_pump::start_background_safe(tick);
}
...
pub(crate) struct PreparedTurn {
    ...
    pub(crate) retry_attachments: Vec<op_editor_core::chat::ChatAttachment>,
}
...
pub(crate) fn prepare_turn(state: &mut EditorState) -> Option<PreparedTurn> {
    ...
    let attachments_json: Vec<serde_json::Value> = state
        ...
        .collect();
    let retry_attachments = std::mem::take(&mut state.chat.pending_attachments);
