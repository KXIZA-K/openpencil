...
pub fn models_json(editor: &EditorState) -> String {
    ...
    for agent in editor
        ...
        .filter(|agent| {
            agent.ready()
                && (!crate::web_credentials::browser_owns_builtin_agent(agent)
                    || managed_model_id(agent).is_some())
        ...
        })
    {
        for model in agent.models.iter().map(|model| model.trim()) {
            ...
            let id = managed_model_id(agent).unwrap_or(&agent.id);
            let (group, label) = if id.starts_with("ids-openai-codex-") {
                ("codex", "Coco Host · Codex")
            } else if id.starts_with("ids-xai-") {
                ("xai", "Coco Host · xAI")
            } else {
            ...
            models.push(json!({
                ...
                "value": format!("builtin:{id}:{model}"),
                "displayName": if managed_model_id(agent).is_some() && agent.models.len() == 1 { agent.display_name.as_str() } else { model },
                "providerDisplayName": if managed_model_id(agent).is_some() { label } else { &agent.display_name },
                "providerGroup": if managed_model_id(agent).is_some() { Some(group) } else { None },
...
fn proxy_builtin_for_identity(
    ...
    if let Some(builtin_id) = builtin_provider_id {
        let mut candidates = editor
            ...
            .builtin_agents
            .iter()
            .filter(|agent| builtin_agent_id_matches(agent, builtin_id));
        ...
        for agent in &editor.editor_ui.agent_settings.builtin_agents {
            ...
            for saved_model in &agent.models {
                if format!("builtin:{}:{saved_model}", agent.id) != model
                    && managed_model_id(agent)
                        .is_none_or(|id| format!("builtin:{id}:{saved_model}") != model)
                {
                    continue;
                }
...
/// Preserve the Platform's stable managed model keys across credential scoping.
fn managed_model_id(agent: &BuiltinAgentConfig) -> Option<&str> {
    let id = agent.id.rsplit(':').next()?;
    (id.starts_with("ids-openai-codex-")
        || id.starts_with("ids-xai-")
        || id.starts_with("ids-ids-"))
    .then_some(id)
}

fn builtin_agent_id_matches(agent: &BuiltinAgentConfig, requested: &str) -> bool {
    agent.id == requested || managed_model_id(agent) == Some(requested)
}
