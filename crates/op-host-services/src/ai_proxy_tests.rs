...
#[test]
fn managed_model_catalog_preserves_platform_key_and_exact_resolution() {
    let mut editor = EditorState::new();
    editor
        .editor_ui
        .agent_settings
        .add_builtin_agent_with_defaults("Central model", "test-key", "managed-model");
    editor.editor_ui.agent_settings.builtin_agents[0].id =
        "web-credential:builtin:ids-ids-fixture".into();
    let rows = model_catalog(&editor);
    assert_eq!(rows[0]["value"], "builtin:ids-ids-fixture:managed-model");
    assert_eq!(rows[0]["builtinProviderId"], "ids-ids-fixture");
    assert_eq!(rows[0]["displayName"], "Central model");
    assert!(exact_provider(
        &editor,
        "ids-ids-fixture",
        "builtin:ids-ids-fixture:managed-model"
    )
    .is_some());
    let duplicate = editor.editor_ui.agent_settings.builtin_agents[0].clone();
    editor
        .editor_ui
        .agent_settings
        .builtin_agents
