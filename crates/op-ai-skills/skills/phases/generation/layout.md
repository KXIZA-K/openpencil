...
PINNED SCROLL / BOTTOM NAV — No CSS `position`. A fixed-height `clipContent: true` frame scrolls only with non-empty `events.onScroll`; fix a direct child via `stickyChildren: ["nav"]` and `pin: true`. Pin is unconditional. Outside it, bottom nav stays inline; never add a trailing spacer.
