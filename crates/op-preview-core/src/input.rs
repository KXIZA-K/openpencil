...
impl PreviewSession {
    ...
    /// (document) space via the editor viewport; here we map it
    /// into the runtime node's absolute scene rect so the hit-test
    /// matches where the widget paints. Returns `true` when the runtime emitted any semantic
    /// event.
    ...
    pub fn dispatch_pointer_for_id_at(
        ...
        self.scope_pointer_hit(hit.as_deref());
    ...
    pub(crate) fn dispatch_pointer_event(
        ...
        self.scope_pointer_hit(hit_node.as_deref());
    ...
    ///
    /// Jian indexes absolute scene coordinates, including the authored root
    /// origin. Use the same geometry here even when the browser normalizes
    /// the painted scene to its viewport origin.
