...
- [x] **R2B1 — Swipe runtime + drag-family arbitration + factual per-event timestamps across clients.** Jian commit `628ab0b` (`feat(renderer): add swipe gesture runtime`, pointer `26e8a257f`) landed Swipe direction/axial distance/velocity/payloads, Pan-over-Swipe priority, the PressCancel parent re-binding guard, Cancel scoped to only the cancelled pointer's LongPress/ContextMenu timers, and deterministic pending Tap/LongPress/Cancel settlement. The root side landed the client timestamp slice: `PreviewSession::dispatch_pointer_phase_at` separates factual event time from the global monotonic clock; `op_editor_press_at/move_at/release_at/cancel_gesture_at` (FFI/JNI/NAPI/C header/types/contracts); Native/Web Preview timestamp forwarding; iOS/Android/Harmony real-editor entrances migrated onto the `_at` entries with synthetic Cancels on the shared monotonic clock and explicit stream cancels before every suspend/background barrier. Verified via `op-preview-core` (97), gl-host native suite (884), dedicated FFI `editor_pointer_clock`, native `preview_swipe`, JNI/NAPI suites, iOS `validate_sources.sh`, and Android/Harmony source contracts. Known limitation: the Harmony artifact gate needs a rebuilt `libopenpencil.so` (no OHOS NDK in this environment). Caveat for R2B2: Preview client input still feeds fixed `id=1`+Mouse with a single capture, so Scale/Rotate co-winning stays engine-tested until R4 makes capture per-pointer-id.
- [ ] **R2B2 — deterministic `Scale → Rotate` ordering.** Fixed evaluation order independent of `interactionOrder` (authoring-only), same-multi-team co-win, preflight-then-one-shot-capture arena protocol without partial cancels, 2→1→2 new-baseline repair with Start/End symmetry, third-finger independence, and HashMap-order-independent output. Requires the R4 per-pointer-id client capture before any client-side completion claim.
  - Engine arbitration LANDED — Jian `e233014` (`feat(renderer): add deterministic scale rotate multi-team arbitration`): fixed order via `priority::multi_claim_order`, preflight-then-one-shot capture where a team's own prior captures (`u64::MAX` sentinel) no longer read as too-late competitor wins, fresh-session quorum bookkeeping in both recognizers, participant-capacity gate plus untracked-Up no-op for stray fingers, and sorted fan-out/timer/teardown snapshots. Covered by `tests/gesture_r2b_transform_arbitration.rs` (6) and recognizer unit regressions; Jian workspace clippy `-D warnings`, 2016 workspace tests, and wasm check green. REMAINING: the R4 Preview core/Native/Web per-pointer-id capture + mobile second-finger-to-Jian routing before any client-side Scale/Rotate completion may be claimed.
  - Preview per-pointer-id capture LANDED (core + both panel hosts): `PreviewSession::dispatch_pointer_for_id_at(pointer_id, kind, …)` replaces the synthetic single-id stream as the primary entry (`dispatch_pointer_phase[_at]` stay as id-1 Mouse compatibility wrappers), the single `gesture_mapping` slot became `gesture_mappings: HashMap<u32, (Rect, Rect)>`, `cancel_pointer(pid)` settles one pointer's arena timers without coordinates for teardown barriers, and both hosts moved their pressed-state to `preview_pressed_pids` / `preview_last_doc_by_pid` with an edge-swipe owner gate (only the arming finger can pop). Proven by `tests_multi_pointer.rs`: two real ids through ONE preview session claim ScaleStart AND RotateStart with symmetric Ends — impossible under the old single-id stream. REMAINING for this task's full contract: the unified `PreviewInput` enum (IME/wheel/key/lifecycle variants + `PreviewDispatchOutcome`/`pump`/capabilities) and the H0/H1 shell gates; on mobile the second-finger-to-Jian fork additionally needs the engine→shell "preview capturing" signal that H1 introduces — not attempted here.
