...
raise "handled extent changes must rebind the EGL window surface" unless surface.match?(/surfaceChanged.*?extentChanged.*?if \(attachedOnce\) OpNative\.nativeSuspend\(engine\).*?attachOrResume\(holder\.surface\)/m)
